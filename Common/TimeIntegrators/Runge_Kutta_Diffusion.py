"""
=========================================================================================
Runge-Kutta Methods for Diffusion
=========================================================================================

This module provides Runge-Kutta time integrators for Diffusion equations.
...
def RungeKutta2_1D(x, T, nu, u, u_ap):
    """
    Second-Order Runge-Kutta (RK2) Method for 1D Diffusion.

    Advances the numerical solution of a 1D Diffusion problem in time 
    ...
    for k in range(t - 1):
        k1 = rhs_1D(u_ap[:, k], T, k, x, nu, u)                                                 # First RK2 slope from the current state.
        k2 = rhs_1D(u_ap[:, k] + (dt / 2) * k1, T + (dt / 2), k, x, nu, u)                      # Second RK2 slope from the midpoint state.
...
def RungeKutta3_1D(x, T, nu, u, u_ap):
    """
    Third-Order Runge-Kutta (RK3) Method for 1D Diffusion.
    ...
    for k in range(t - 1):
        k1 = rhs_1D(u_ap[:, k], T, k, x, nu, u)                                                 # First RK3 slope from the current state.
        k2 = rhs_1D(u_ap[:, k] + (dt / 2) * k1, T + (dt / 2), k, x, nu, u)                      # Second RK3 slope from the midpoint state.
        k3 = rhs_1D(u_ap[:, k] + (2 * dt) * (k2 - k1), T + dt, k, x, nu, u)                     # Third RK3 slope from the endpoint predictor.
...
def RungeKutta4_1D(x, T, nu, u, u_ap):
    """
    Fourth-Order Runge-Kutta (RK4) Method for 1D Diffusion.
    ...
    for k in range(t - 1):
        k1 = rhs_1D(u_ap[:, k], T, k, x, nu, u)                                                 # First RK4 slope from the current state.
        k2 = rhs_1D(u_ap[:, k] + (dt / 2) * k1, T + (dt / 2), k, x, nu, u)                      # Second RK4 slope from the first midpoint state.
        k3 = rhs_1D(u_ap[:, k] + (dt / 2) * k2, T + (dt / 2), k, x, nu, u)                      # Third RK4 slope from the second midpoint state.
        k4 = rhs_1D(u_ap[:, k] + dt * k3, T + dt, k, x, nu, u)                                  # Fourth RK4 slope from the endpoint state.
...
def rhs_1D(u_ap, T, k, x, nu, u):
    """
    Right-Hand Side (RHS) of the 1D Diffusion ODE system.
    ...
    for i in range(1, m - 1):
        s[i] = (nu / dx**2) * (u_ap[i - 1] - 2 * u_ap[i] + u_ap[i + 1])                         # Centered second derivative scaled by diffusivity.
    return s

def RungeKutta2_2D(x, y, T, nu, u, u_ap):
    """
    Second-Order Runge-Kutta (RK2) Method for 2D Diffusion.

    Advances the numerical solution of a 2D Diffusion problem in time 
    using a second-order Runge-Kutta scheme (Method of Lines).
    ...
    for k in range(t - 1):
        k1 = rhs_2D(u_ap[:, :, k], T, k, x, y, nu, u)                                           # First RK2 slope from the current 2D state.
        k2 = rhs_2D(u_ap[:, :, k] + (dt / 2) * k1, T + (dt / 2), k, x, y, nu, u)                # Second RK2 slope from the midpoint 2D state.
...
def RungeKutta3_2D(x, y, T, nu, u, u_ap):
    """
    Third-Order Runge-Kutta (RK3) Method for 2D Diffusion.
    ...
    for k in range(t - 1):
        k1 = rhs_2D(u_ap[:, :, k], T, k, x, y, nu, u)                                           # First RK3 slope from the current 2D state.
        k2 = rhs_2D(u_ap[:, :, k] + (dt / 2) * k1, T + (dt / 2), k, x, y, nu, u)                # Second RK3 slope from the midpoint 2D state.
        k3 = rhs_2D(u_ap[:, :, k] + dt * (2 * k2 - k1), T + dt, k, x, y, nu, u)                 # Third RK3 slope from the endpoint predictor.
...
def RungeKutta4_2D(x, y, T, nu, u, u_ap):
    """
    Fourth-Order Runge-Kutta (RK4) Method for 2D Diffusion.
    ...
    for k in range(t - 1):
        k1 = rhs_2D(u_ap[:, :, k], T, k, x, y, nu, u)                                           # First RK4 slope from the current 2D state.
        k2 = rhs_2D(u_ap[:, :, k] + (dt / 2) * k1, T + (dt / 2), k, x, y, nu, u)                # Second RK4 slope from the first midpoint state.
        k3 = rhs_2D(u_ap[:, :, k] + (dt / 2) * k2, T + (dt / 2), k, x, y, nu, u)                # Third RK4 slope from the second midpoint state.
        k4 = rhs_2D(u_ap[:, :, k] + dt * k3, T + dt, k, x, y, nu, u)                            # Fourth RK4 slope from the endpoint state.
...
def rhs_2D(u_ap, T, k, x, y, nu, u):
    """
    Right-Hand Side (RHS) of the 2D Diffusion ODE system.
    ...
    for i in range(1, m - 1):
        for j in range(1, n - 1):
            s[i, j] = (                                                                         # Combine x and y diffusion contributions at node (i, j).
                (nu / dx**2) * (u_ap[i, j - 1] - 2 * u_ap[i, j] + u_ap[i, j + 1])               # Centered second derivative in x.
                + (nu / dy**2) * (u_ap[i - 1, j] - 2 * u_ap[i, j] + u_ap[i + 1, j])             # Centered second derivative in y.
            )
...
if __name__ == "__main__":
    print("This module defines Runge-Kutta helpers for diffusion. Import it from solver or test modules.")  # Inform users that this file provides reusable integrators.
