...
def RungeKutta3_1D(x, T, nu, u, u_ap):
    ...
    for k in range(t-1):
        k1 = rhs_1D(u_ap[:,k],              T,        k, x, nu, u)
        k2 = rhs_1D(u_ap[:,k]+(dt/2)*k1,    T+(dt/2), k, x, nu, u)
        k3 = rhs_1D(u_ap[:,k]+dt*(2*k2-k1), T+dt,     k, x, nu, u)
...
def rhs_2D(u_ap, T, k, x, y, nu, u):
    ...
    for i in range(m):
        s[i,0]  = u(x[i,0],  y[i,0], T[k],nu)                      # Boundary condition at y = 0.
        s[i,-1] = u(x[i,-1], y[i,-1],T[k],nu)                      # Boundary condition at y = 1.
    for j in range(n):
        s[0,j]  = u(x[0,j],  y[0,j], T[k],nu)                      # Boundary condition at x = 0.
        s[-1,j] = u(x[-1,j], y[-1,j],T[k],nu)                      # Boundary condition at x = 1.
