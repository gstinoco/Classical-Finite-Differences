'''
Runge-Kutta Methods for MOL in Advection-Diffusion problems.
...
def RungeKutta2_1D(x, T, nu, a, u, u_ap):
    ...
    for k in range(t-1):
        k1 = rhs_1D(u_ap[:, k],             T,          k, x, nu, a, u)
        k2 = rhs_1D(u_ap[:, k] + (dt/2)*k1, T + (dt/2), k, x, nu, a, u)
...
def RungeKutta3_1D(x, T, nu, a, u, u_ap):
    ...
    for k in range(t-1):
        k1 = rhs_1D(u_ap[:, k],                    T,          k, x, nu, a, u)
        k2 = rhs_1D(u_ap[:, k] + (dt/2)*k1,        T + (dt/2), k, x, nu, a, u)
        k3 = rhs_1D(u_ap[:, k] + (2*dt)*(k2 - k1), T + dt,     k, x, nu, a, u)
...
def RungeKutta4_1D(x, T, nu, a, u, u_ap):
    ...
    for k in range(t-1):
        k1 = rhs_1D(u_ap[:, k],                T,          k, x, nu, a, u)
        k2 = rhs_1D(u_ap[:, k] + (dt/2)*k1,    T + (dt/2), k, x, nu, a, u)
        k3 = rhs_1D(u_ap[:, k] + (dt/2)*k2,    T + (dt/2), k, x, nu, a, u)
        k4 = rhs_1D(u_ap[:, k] + dt*k3,        T + dt,     k, x, nu, a, u)
...
def RungeKutta2_2D(x, y, T, nu, a, b, u, u_ap):
    ...
    for k in range(t-1):
        k1 = rhs_2D(u_ap[:, :, k],             T,          k, x, y, nu, a, b, u)
        k2 = rhs_2D(u_ap[:, :, k] + (dt/2)*k1, T + (dt/2), k, x, y, nu, a, b, u)
...
def RungeKutta3_2D(x, y, T, nu, a, b, u, u_ap):
    ...
    for k in range(t-1):
        k1 = rhs_2D(u_ap[:, :, k],                T,          k, x, y, nu, a, b, u)
        k2 = rhs_2D(u_ap[:, :, k] + (dt/2)*k1,    T + (dt/2), k, x, y, nu, a, b, u)
        k3 = rhs_2D(u_ap[:, :, k] + dt*(2*k2-k1), T + dt,     k, x, y, nu, a, b, u)
...
def RungeKutta4_2D(x, y, T, nu, a, b, u, u_ap):
    ...
    for k in range(t-1):
        k1 = rhs_2D(u_ap[:, :, k],                T,          k, x, y, nu, a, b, u)
        k2 = rhs_2D(u_ap[:, :, k] + (dt/2)*k1,    T + (dt/2), k, x, y, nu, a, b, u)
        k3 = rhs_2D(u_ap[:, :, k] + (dt/2)*k2,    T + (dt/2), k, x, y, nu, a, b, u)
        k4 = rhs_2D(u_ap[:, :, k] + dt*k3,        T + dt,     k, x, y, nu, a, b, u)
...
def rhs_2D(u_ap, T, k, x, y, nu, a, b, u):
    ...
    for i in range(m):
        s[i, 0]  = u(x[i, 0],  y[i, 0],  T[k], nu, a, b)
        s[i, -1] = u(x[i, -1], y[i, -1], T[k], nu, a, b)
    for j in range(n):
        s[0, j]  = u(x[0, j],  y[0, j],  T[k], nu, a, b)
        s[-1, j] = u(x[-1, j], y[-1, j], T[k], nu, a, b)
    ...
    for i in range(1, m-1):
        for j in range(1, n-1):
            s[i, j] = (nu/dx**2)*(u_ap[i-1, j]   - 2*u_ap[i, j] + u_ap[i+1, j]) + \
                      ...
                      (a/(2*dx))*(u_ap[i+1, j]   - u_ap[i-1, j]) - \
                      (b/(2*dy))*(u_ap[i,   j+1] - u_ap[i,   j-1])
